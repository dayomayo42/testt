...
Основано на [https://github.com/tutu-ru/swift-style-guide](https://github.com/tutu-ru/swift-style-guide)
...
*   Следует именовать параметры кортежей (tuples), потому что потом при использовании кортежей код будет более понятен
...
Следует снабжать комментариями public/internal поля и методы структур, классов и протоколов. Также следует давать комментарии самим структурам/полям/протоколам. Исключения — реализация уже прокомментированного протокола в классе/структуре, вспомогательные методы в классах структурах, если их код простой, а именование говорит само за себя.

Комментирование следует по возможности осуществлять комбинацией клавиш "cmd+shift+/", если разработка ведется в Xcode.
...
```
## Вложенные типы
*Рекомендациями этого раздела следует пользоваться только следующих ситуациях:*
...
```
## Организация кода
Каждый логический блок функциональности следует помечать `// MARK: -` для придания большей читаемости кода.
> Внутри скоупа `MARK:` следует писать без `-`, для того, чтобы не было разделителей внутри.
### Protocol Conformance
Когда класс конформит несколько протоколов их необходимов разбивать в разные extensions или выделить при помощи `// MARK:`. Такой подход поможет сгруппировать родственные методы вместе с протоколом, который они конформят.
...
```
Если мы разворачиваем сразу несколько опционалов, то лучше делать это одной командой:
```swift
